...
Duration TimeLimitedStrategy::get_iteration_time_limit(const Context& context) const {
    ...
    const auto max_time_for_current_iteration = Ms(Ms(get_full_time_limit(tick, is_master) - sum_time_).count() * 0.7);
    ...
    return Ms(std::min(std::min(Ms(max_time_for_current_iteration).count(), Ms(recommended).count()) * 0.7, Ms(1000).count()));
}
