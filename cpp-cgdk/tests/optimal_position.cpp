...
TEST(get_optimal_position, for_all_default) {
    ...
    ASSERT_EQ(get_optimal_position(context, target), get_position(wizard));
}
...
TEST(get_optimal_position, for_me_and_enemy_wizard) {
    ...
    const auto result = get_optimal_position(context, &target);
    ASSERT_LE(result.distance(get_position(target)) - (GAME.getWizardCastRange() + GAME.getMagicMissileRadius()), 1);
    ASSERT_EQ(result, Point(754.08414214017716, 724.97268783975505));
}
