#ifndef STRATEGY_COMMON_HPP
#define STRATEGY_COMMON_HPP

namespace strategy {

using UnitId = long long;
using Tick = int;

template <class T>
class Id {
public:
    using Type = T;

    Id(UnitId value = 0) : value_(value) {}

    UnitId value() const {
        return value_;
    }

private:
    UnitId value_;
};

const Tick MESSAGE_TICKS = 20000;
const int OPTIMAL_PATH_STEP_SIZE = 20;
const Tick OPTIMAL_PATH_MAX_TICKS = 50;
const Tick OPTIMAL_PATH_MAX_ITERATIONS = 1000;
const long OPTIMAL_POSITION_MINIMIZE_MAX_FUNCTION_CALLS = 200;
const double UNITS_DANGER_PENALTY_WEIGHT = 1.2;
const double UNITS_COLLISION_PENALTY_WEIGHT = 1.1;
const double BONUSES_PENALTY_WEIGHT = 0.5;
const double PROJECTILE_PENALTY_WEIGHT = 1.3;
const double FRIENDLY_FIRE_PENALTY_WEIGHT = 0.1;
const double TARGET_PENALTY_WEIGHT = 0.05;
const double BORDERS_PENALTY_WEIGHT = 1.4;
const double ELIMINATION_SCORE_WEIGHT = 1;
const Tick BATTLE_MODE_TICKS = 2500;
const Tick TICKS_TO_DEATH_FOR_RETREAT = 150;

}

#endif
